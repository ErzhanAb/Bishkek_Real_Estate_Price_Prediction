...
Для расчета доверительного интервала использован:
...
*   `Bagging` позволяет дополнить модель доверительным интервалом.
...
*   Доверительный интервал считается по бутстрэпу деревьев (используются предсказания всех деревьев ансамбля).
...
Доверительный интервал считается по всем деревьям ансамбля.
...
*   **Квантильные модели:**
    *   `CatBoost_lower.pkl` — нижняя граница 95% доверительного интервала (2.5-й перцентиль).
    *   `CatBoost_upper.pkl` — верхняя граница 95% доверительного интервала (97.5-й перцентиль).
...
Для доверительных интервалов использовались те же параметры, но с другими функциями потерь:
...
*   Квантильные модели позволяют получать не только медианное предсказание, но и 95% доверительный интервал.
